...
export default function POS() {
    ...
    const nasiyaAccounts = useMemo(
        () =>
            creditAccounts.filter(
                (a) => String(a.businessId) === String(currentBusinessId)
            ),
    ...
    const handleNasiyaSale = async () => {
        ...
        const ok = await checkout({
            ...
            customerPhone: selectedNasiyaAccount.phone || '', // telefon raqami
            creditAccountId: selectedNasiyaAccount.id, // <--- MANA SHU BACKENDGA ID NI YUBORADI
        });
        if (ok) {
            setNasiyaOpen(false);
            resetNasiyaForm();
    ...
    return (
        <div className="flex flex-col h-full w-full overflow-hidden">
            ...
            <div className="shrink-0 flex items-center justify-between text-white px-4 py-3 shadow-md gap-3"
                 style={{background: `linear-gradient(90deg, ${PRIMARY} 0%, ${PRIMARY_HOVER} 100%)`}}>
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden">
                ...
                >
                    <div className="w-[280px] h-full flex flex-col">
                        <div className="px-4 py-4 border-b border-white/10 flex items-center justify-between shrink-0">
                            ...
                            <IconButton size="small" onClick={() => setMenuOpen(false)} aria-label="Menyuni yopish"
                ...
                <div className="flex flex-1 min-h-0 gap-3 p-3 overflow-hidden min-w-0" style={{background: PAGE_BG}}>
                    {/* Mahsulotlar */}
                    <div
                        className="flex-[1.8] flex flex-col bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm min-w-0">
                        ...
                        <div
                            className="flex-1 min-h-0 overflow-y-auto p-3 grid grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-3 content-start auto-rows-min"
                            style={{background: PAGE_BG}}>
                            {filtered.length === 0 ? (
                                <div className="col-span-full text-center py-20 text-gray-400 text-lg">Mahsulot
                                    topilmadi</div>
                            ) : (
                                filtered.map((p) => {
                                    const {
                                    ...
                                    } = productAvailability.get(p.id) || {physical: 0, available: 0};
                        ...
                        {available} ta
                                            ...
                                            <div className="flex-1 flex items-center justify-center py-2 min-h-[88px]">
                                                <div
                                                    className="w-[72px] h-[72px] rounded-xl bg-gray-50 flex items-center justify-center border border-gray-100 overflow-hidden">
                                                    {p.image ? (
                                                        <img src={p.image} alt=""
                                            ...
                                            <div className="pt-2 border-t border-gray-100">
                                                ...
                                                {formatProductSizeDisplay(p) && (
                                                    <p className="text-xs font-semibold truncate mt-0.5"
                                                       style={{color: PRIMARY}}>{formatProductSizeDisplay(p)}</p>
                                                )}
                                                <div className="flex items-end justify-between mt-1.5 gap-2">
                                                    <p className="text-lg font-black leading-none"
                                                       style={{color: PRIMARY}}>{fmtNum(p.price)}</p>
                                                    {available <= 0 && physical > 0 && (
                                                        <span
                                                            className="text-xs font-bold px-2 py-1 rounded-lg bg-amber-50 text-amber-700">Band</span>
                                                    )}
                                                    {available <= 0 && physical <= 0 && (
                                                        <span
                                                            className="text-xs font-bold px-2 py-1 rounded-lg bg-red-50 text-red-600">Tugagan</span>
                                                    )}
                                                </div>
                                            </div>
                                        </button>
                    ...
                    {/* Savat */}
                    <div
                        className="w-[min(480px,42vw)] shrink-0 flex flex-col bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                        <div className="p-4 bg-[#f8fafc] border-b border-gray-100 space-y-4">
                            <div className="flex items-center justify-between gap-2">
                                ...
                                <span
                                    className="text-base font-extrabold text-white min-w-[36px] h-9 px-3 flex items-center justify-center rounded-full"
                                    style={{background: PRIMARY}}>
                {cart.length}
              </span>
                            </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <Badge badgeContent={posDrafts.length} color="warning" max={99}
                                       sx={{flex: 1, '& .MuiBadge-badge': {fontSize: 12, height: 22, minWidth: 22}}}>
                                    <Button
                                        fullWidth
                                        variant="contained"
                        ...
                        <div className="flex-1 overflow-y-auto bg-white">
                            {cart.length === 0 ? (
                                <div
                                    className="flex flex-col items-center justify-center h-full text-gray-400 p-8 text-center space-y-3">
                                    <CategoryOutlined style={{fontSize: 52, color: '#cbd5e1'}}/>
                                    <p className="text-base font-semibold text-gray-500">Savat hozircha bo&apos;sh</p>
                                    <p className="text-sm text-gray-400">Mahsulot kartasini bosing yoki shtrix-kod
                                        skaner qiling</p>
                                </div>
                            ) : (
                                <div className="divide-y divide-gray-100">
                                    {cart.map((item) => (
                                        <div key={item.id}
                                             className="flex items-center gap-3 px-3 py-3 hover:bg-gray-50/60">
                                            <div className="min-w-0 flex-1">
                                                ...
                                                {formatProductSizeDisplay(item) && (
                                                    <p className="text-xs font-semibold truncate"
                                                       style={{color: PRIMARY}}>{formatProductSizeDisplay(item)}</p>
                                                )}
                                                <p className="text-sm text-gray-500 mt-0.5">{fmt(item.price)}</p>
                                            </div>
                                            <div
                                                className="flex items-center gap-1 border-2 border-gray-200 rounded-2xl bg-white p-1 shrink-0">
                                                <button
                                                    type="button"
                            ...
                            <div className="flex justify-between items-end px-1 pb-1">
                                ...
                                <div className="text-right">
                                    <span className="font-black text-3xl leading-none block"
                                          style={{color: PRIMARY}}>{fmtNum(total)}</span>
                                    <span className="text-sm text-gray-400 mt-1 block">so&apos;m</span>
                                </div>
                            </div>

                            <div className="space-y-3">
                                <p className="text-xs uppercase font-bold text-gray-400 tracking-wide">To&apos;lov
                                    usuli</p>
            ...
            {receiptDialog && (
                ...
                >
                    <div
                        className="flex flex-col items-center gap-4 max-h-[95vh] overflow-y-auto print:overflow-visible">
                        <div
                            id="pos-receipt-slip"
            ...
            <Dialog open={mixOpen} onClose={() => setMixOpen(false)} maxWidth="xs" fullWidth>
                ...
                <DialogContent>
                    ...
                    <p className="text-xs text-gray-400 mt-2">
                        Masalan: {fmtNum(Math.round(total * 0.3))} naqd
                        + {fmtNum(total - Math.round(total * 0.3))} karta
                    </p>
                </DialogContent>
                <DialogActions sx={{px: 3, pb: 2}}>
                                    ...
                                    <li {...props} key={option.id}>
                                        <div className="py-0.5">
                                            ...
                                            <p className="text-xs text-gray-500">
                                                <span
                                                    className="text-amber-700 font-bold">{fmtNum(option.balance)} so&apos;m qarz</span>
                                            </p>
                                        </div>
                                    </li>
                                )}
                    ...
                    <div>
                        ...
                        <div className="space-y-4 text-xs text-gray-300 leading-relaxed">
                            ...
                            <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                ...
                                <p>O'ng tomondagi ro'yxatdan mahsulotlarni tanlab savatga qo'shing. To'lov usulini
                                    belgilang va "Sotish" tugmasini bosing.</p>
                            </div>

                            <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                <p className="font-bold text-purple-400 mb-1">📦 Sklad / Prixod nazorati</p>
                                <p>Dilerga buyurtma (Zakaz) bering. Prixod bo'limidan kelgan mahsulotlarni qabul qiling,
                                    bu mahsulotlar avtomatik ravishda ombor qoldig'iga qo'shiladi.</p>
                            </div>

                            <div className="bg-white/5 p-3 rounded-lg border border-white/10">
                                <p className="font-bold text-purple-400 mb-1">👥 Agentlar va CRM</p>
                                <p>Mijozlarni ro'yxatga oling. Agentlar orqali buyurtmalarni tarqating va ularning
                                    statistikasini alohida kuzating.</p>
                            </div>
                        </div>
                    </div>
