...
export default function Sidebar() {
  ...
  const mainNav = filterByRole(NAV_MAIN, role, isGlobalAdmin);
  const bottomNav = filterByRole(NAV_BOTTOM, role, isGlobalAdmin);
  const navGroups = filterGroupsByRole(NAV_GROUPS, role, isGlobalAdmin);
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 py-4 border-b border-white/10">
        ...
        {collapsed && (
          <BrandLogo size={34} iconOnly className="mx-auto mb-1" />
        )}
        {collapsed && (
          <IconButton
          ...
          {!collapsed && (
            <div className="flex-1 text-left">
              ...
              <p className="text-xs" style={{ color: 'rgba(255,255,255,0.45)' }}>
                {isGlobalAdmin ? 'Platform egasi (Superadmin)' : (ROLE_LABELS[currentUser?.role] ?? '')}
              </p>
            </div>
          )}
          {!collapsed &&
